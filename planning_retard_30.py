...
def insert():
    ...
    cours = datetime.now().replace(microsecond=0) - timedelta(minutes=40)
    now = datetime.now().replace(microsecond=0)
    fin = now + timedelta(hours=3)
    
    cur.execute(
        "INSERT INTO planning (Site, Salle, DateDebut, DateFin, Filiere, Promotion, idIntervenant, Matiere) VALUES ('Fraissinette', 'E T13', '" + str(
            cours) + "', '" + str(fin) + "', 'SN', '17', '1', 'Réseau')")
